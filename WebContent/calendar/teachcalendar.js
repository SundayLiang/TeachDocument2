$(document).ready(function() {
	...
	function refreshAllData(json, toPage) {
		...
		for (var i = 0; i < rowList.length; i++) {
			...
			temp += "<input type='button' value='详细' onclick='showDetail(\""+dirs+"\","+rowList[i].columns.id+",\""+rowList[i].columns.term+"\")' />"; 
			temp += "</td></tr>";
		}
...
function showDetail(dirs,taskID,term){
	parent.$("#mainFrame").attr("src","calendar/docdetail.html?taskID="+taskID+"&term="+term+"&dirs="+dirs);
}
